 ...
 ******************************************************************************
   ...
   the c_class is taken as closed and will be not be considered for unknown
...
{
    Mapping next;
    char   *c_class;	/* Printing form of the tag */
    BOOL    closed;	/* Closed tag c_class (main map list) */
    BOOL    phrasal;	/* Phrasal tag c_class (main map list) */
...
static int tag_compare(const void *c1, const void *c2)
{
    Map s1 = (Map)c1;
    Map s2 = (Map)c2;
    return strcmp(s1->c_class, s2->c_class);
}
...
static BOOL is_ditto_tag(char *text)
{
...
static int add_tag_to_list(Mapping *pmap_head, char *tag, int ntags,
			   BOOL closed, BOOL phrasal, Tag to)
{
    Mapping c_new;

    /* Search and insert into list */
    c_new = (Mapping)list_search_and_check((void **)pmap_head, (Key *)&tag, map_cmp,
				sizeof(MappingSt), "map list", tag);

    if (c_new != NULL)
    {
	c_new->closed  = closed;
	c_new->phrasal = phrasal;
	c_new->to      = to;
	c_new->c_class   = allocate_string(tag, "map c_class");
...
static Tag map_tag_in_list(char *form, Map list, int max, BOOL report)
{
    MapSt m, *r;
...
void read_mapping(char *name)
{
	...
	*(end+1) = '\0';
	...
	{
	    /* If the first character is a dash, this is a closed c_class */
	    /* If the first character is a plus, this is a phrasal c_class */
#ifdef Phrasal
	    phrasal = (buffer[0] == '+');
#else
...
/* Failure string */
static char *nomap_string = "UNDEFINED ";

char *unmap_tag(Tag code)
{
#ifdef Phrasal
...
static Tag map_tag_report(char *form, BOOL report)
{
...
{
    FILE    *fp;
    char   buffer[MAXTAG], fromtag[MAXTAG], totag[MAXTAG];
...
static char skip_spaces( FILE *fp )
{
    while ( !feof(fp) )
    {
	int c = getc(fp);
...
void read_unknown_rules(char *name)
{
	...
	/* Make the c_new rule */
	Allocate( c_new_rule, sizeof(UnkRuleSt), "unknown rule" );
	if ( unknown_word_rules == NULL )
	{
	    unknown_word_rules = c_new_rule;
	    c_new_rule->next = NULL;
	}
	else
	    rule->next = c_new_rule;

	rule = c_new_rule;

	/* Clear out the rule */
	rule->number = rule->cont = -1;
	rule->c_class.negated_pattern = rule->c_class.negated_tags =
	rule->c_class.prefix = rule->c_class.suffix =
	rule->c_class.initial_cap = rule->c_class.all_cap =
	rule->c_class.any_cap = 0;
	...
	if ( c == '~' )
	{
	    rule->c_class.negated_pattern = 1;
	    if ( (c = skip_spaces( fp )) == 0 ) { abrupt_end = TRUE; break; }
	}

	/* Check for special patterns */
	if ( c == 'I' ) rule->c_class.initial_cap = 1;
	else if ( c == 'A' ) rule->c_class.all_cap = 1;
	else if ( c == 'C' ) rule->c_class.any_cap = 1;
	else if ( c == 'D' ) rule->c_class.cardinal_number = 1;
	else if ( c == 'T' ) rule->c_class.time_format = 1;
	else if ( c == 'O' ) 
	{
	    rule->c_class.ordinal_number = 1;
