...
static Sentence insert_sentence(Sentence sent)
{
	...
	for (i = 0 ; i < nerrors ; i++, f = f->next)
	{
	    ...
	    Allocate(c_newsent, sizeof(SentenceSt), "sentence");
	    memcpy(c_newsent, sent, sizeof(SentenceSt));
	    c_newsent->error = f;
	    l->usage += 1;
			 ...
			 Tag ctag, Tag chosen)
{
    ...
    /* If we hit an anchor, then start a c_new sentence */
    if (word == NULL || is_anchor(WordText(word)) ||
	(InOpt(penn_treebank) && strcmp((char *)WordText(word), ".") == 0))
    {
    ...
    Allocate(c_new, sizeof(SentSt), "sentence word");
    c_new->next = NULL;
    c_new->ambig    = ambig;
    c_new->wordkind = wordkind;
    c_new->correct  = correct;
    if (word->text == NULL)
    {
	c_new->text  = word->d->text;
	c_new->alloc = FALSE;
    }
    else
    {
	c_new->text  = allocate_string(word->text, "sentence word text");
	c_new->alloc = TRUE;
    }
    c_new->ctag   = ctag;
    c_new->chosen = chosen;
