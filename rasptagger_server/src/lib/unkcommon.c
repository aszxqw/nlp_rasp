...
void reallocate(void **c_newmem, void *object, int size, char *text)
{
  fprintf(stderr, "reallocate: size=%d in %s\n", size, text);
    if ((*c_newmem = realloc(object, size)) == NULL)
...
char *compress_word( char *text )
{
  ...
  Allocate(comptext, ((int)strlen(text)+1)*sizeof(char), "comptext: compress_word");
...
char *downcase( char *vanilla_text )
{
  int   i;
  char *text;
  ...
  if ( !is_allalpha( text ) && contains_alpha( text ) && !contains_numbers( text ) )
      ...
      if ( (i != 0) && isalpha(text[i]) )
	{
	  if ( (textlen-i) >= MinTestLen )
	    {
	      c_new_text = string_dup( text );
	      strcpy( (char *)c_new_text, (char *)(&text[i]) );
	    }
	}
    }
	  ...
	  if ( text[j] == '\0' ) /* No consecutive character repeats. */
	    {
	      if ( i >= MinTestLen )
	        {
		  c_new_text = string_dup( text );
		  strncpy( (char *)c_new_text, (char *)text, i );
		  c_new_text[i] = '\0';
	        }
	    }
	}
  ...
  if ( !is_allalpha( text ) && contains_alpha( text ) && !contains_numbers( text ) )
    {
	  ...
	  if ( j > i ) /* Repeating characters exist. */
	    {
	      c_new_text = string_dup( text );
	      strncpy( (char *)c_new_text, (char *)text+i, (j - i + 1));
	      c_new_text[(j - i + 1)] = '\0';
	      break;
	    }
...
char *add_chars( char *text, char* addchars )
{
  ...
  Allocate(add_text, ((int)strlen((char *)text)+(int)strlen((char *)addchars)+1)*sizeof(char), "add_text: add_chars");
...
void set_modifier( char *modifier, BOOL true_capital, BOOL pseudo_capital, BOOL mixed_capital )
{
  int  i;
...
{
  char *smart_cut = NULL, *smart_suffix = NULL, *plus_suffix = NULL;
...
BOOL get_variable_suffix( char *text, char **affix )
{
  ...
  Allocate(*affix, (textlen + 1) * sizeof(char), "affix: get_variable_suffix");
...
BOOL get_affix( int mode, char *text, char **affix, int affixlen, char **remainder )
{
  BOOL success = FALSE;
  ...
  if ( (affixlen > 0) && ((int)strlen( text ) > affixlen) )
    {
      ...
      Allocate( *affix, (affixlen+2)*sizeof(char), "affix: get_affix" );
      ...
      if ( mode == PREFIX )
	{
	  ...
	  if ( remainder != NULL )
	    {
	      Allocate(*remainder, (textlen+1)*sizeof(char), "remainder: get_affix");
...
BOOL split_word( int mode, char *text, char **c_new_prefix_word, int splitlen, char **c_new_suffix_word, BOOL skip_separator )
{
  BOOL success = FALSE;
  ...
  if ( (splitlen > 0) && ((int)strlen( text ) > (splitlen+1)) )
    {
      ...
      Allocate(*c_new_suffix_word, (textlen+1)*sizeof(char), "c_new_suffix_word: split_word");
      Allocate(*c_new_prefix_word, (textlen+1)*sizeof(char), "c_new_prefix_word: split_word");

      (*c_new_prefix_word)[splitlen] = '\0';

      if ( mode == PREFIX )
	{
	  strncpy( (char *)(*c_new_prefix_word), (char *)text, splitlen );
	  
	  if ( (*c_new_suffix_word) != NULL )
	    {
	      strcpy( (char *)(*c_new_suffix_word), ((char *)text)+splitlen+skip_separator );
	    }
	}
      else if ( mode == SUFFIX )
	{
	  ...
	  strncpy( (char *)(*c_new_prefix_word), (char *)(text+offset+skip_separator), splitlen );

	  if ( (*c_new_suffix_word) != NULL )
	    {
	      strncpy( (char *)(*c_new_suffix_word), (char *)text, offset );
	      (*c_new_suffix_word)[offset] = '\0';
	    }
	}
    }
...
TagCut verify_cut( CutList cut_list, BOOL true_capital, BOOL mixed_capital, char *cut )
{
  ...
  CutTagSt   search_cut;
  char      modifier[MAX_MODIFIER_LEN];
...
BOOL get_root( char *text, char **root, int preflen, int sufflen, char **prefix, char **suffix )
{
  BOOL success = FALSE;
  int  textlen;
  char *remainder;
  ...
  for ( tagscore = source; tagscore != NULL; tagscore = tagscore->next )
    {
      ...
      mod_tagscore->score = tagscore->score * (c_new_score / total_score);
...
void add_integrated_transform( TagTrans *start, TagScore orig_taglist, TagScore c_new_tagscore )
{
  BOOL     looking = TRUE, c_new_is_open_tag = !is_closed(c_new_tagscore->tag);
  TagTrans tag_list = *start;  /* Point to start of chain */
  TagTrans last_tag = NULL, next_tag;
  int      skip_flag;

/* Set skip flag, i.e., if the source tag is open c_class, none of the transformed tags can be closed c_class. */
  skip_flag = NO_SKIP_CLOSED_TAGS;
  if ( c_new_is_open_tag )
    {
      skip_flag = SKIP_CLOSED_TAGS;
    }

/*
 * Look through transform chain until we reach the transform tag, the end or a transform tag bigger than the c_new one
*/
  while ( looking )
    {
      if ( tag_list == NULL		        /* End of chain */
	   || tag_list->source_tag->tag > c_new_tagscore->tag )	/* Insert before */
	{
	  next_tag = tag_list;
	  ...
	  add_chain_tags( ADD, &(tag_list->source_tag), NULL, c_new_tagscore, NO_SKIP_CLOSED_TAGS, NULL, "add_integrated_transform1" );
      ...
      else if ( tag_list->source_tag->tag == c_new_tagscore->tag ) /* Entry exists */
	{
	  /* Scan list of tags and increment score or allocate c_new tag, as appropriate */
	  add_chain_tags( ADD, &(tag_list->source_tag), NULL, c_new_tagscore, NO_SKIP_CLOSED_TAGS, NULL, "add_integrated_transform3" );
...
void add_integrated_transforms( TagTrans *start, TagScore orig_taglist, TagScore c_new_taglist )
{
  TagScore tagscore;

  for ( tagscore = c_new_taglist; tagscore != NULL; tagscore = tagscore->next )
    {
...
BOOL check_transform_tags( TagTrans transform_list, TagScore c_new_tagscore )
{
  TagTrans   this_transform = NULL;
  TagScore   next_tagscore;

  for ( next_tagscore = c_new_tagscore; (next_tagscore != NULL) && (this_transform == NULL); next_tagscore = next_tagscore->next )
    {
...
BOOL get_transform_tags( TagTrans transform_list, TagScore c_new_tagscore, TagScore *transform_tags )
{
  ...
  total_score = get_total_score( c_new_tagscore, NULL, NULL );

  for ( next_tagscore = c_new_tagscore; next_tagscore != NULL; next_tagscore = next_tagscore->next )
    {
...
BOOL lookup_enclosure( char *text, int testpos, IndexList indexlist, TagScore *enclosure_tags )
{
  BOOL       success = FALSE;
  IndexWord  scan_word;
  Index      this_index = NULL;
...
TagCut evaluate_replacement_suffix( CutList cut_list, IndexWord scan_word, BOOL true_capital, BOOL mixed_capital, int baselen, int scanlen, char *text, char *scan_text )
{
  BOOL   tags_gotten = FALSE;
  TagCut this_cut = NULL;
  char  *local_cut = NULL, *base_suffix = NULL, *scan_suffix = NULL;
...
TagCut evaluate_container_suffix( CutList cut_list, TagScore scan_tagscore, BOOL true_capital, BOOL mixed_capital, int cutlen, char *scan_text )
{
  BOOL   tags_gotten = FALSE;
  TagCut this_cut = NULL;
  char  *local_cut = NULL;
...
void add_eval_cut( EvalList *start, BOOL scan_mixed_capital, IndexWord scan_word, int baselen, int scanlen, char *scan_text )
{
...
void merge_eval_list( TagScore *cut_tags, EvalList eval_list, CutList cut_list, BOOL true_capital, BOOL mixed_capital, char *text )
{
  EvalList next_eval;
  TagCut   this_cut = NULL;
...
/*----------------------------------------------------------------------------
    ...
    Get a list of cuts and the c_new word, based on part of the input text and on a specified mode.
----------------------------------------------------------------------------*/

TagCut get_partial_cut( int mode, CutList cut_list, BOOL true_capital, BOOL mixed_capital, char *text, int testpos, TagScore *cut_tags )
{
...
char *get_cut( int mode, IndexList indexlist, Dict *dict, char *text, int testpos, 
		int mincutlen, int maxcutlen, DictWord *c_new_dictword, Trans *trans, DB *dbp, Dict *asc )
{
  BOOL  halt_cutting = FALSE;
  char *c_new_word, *local_cut = NULL, *smart_cut;
  ...
  *c_new_dictword = NULL;
  ...
  for ( affixlen = mincutlen; (affixlen <= maxcutlen) && (!halt_cutting) && (*c_new_dictword == NULL); 
	affixlen++ )
    {
      BOOL success = FALSE;
	  ...
	  if ( !halt_cutting )
	    {
	      ...
	      success = get_affix( PREFIX, (text+testpos), &local_cut, affixlen, &c_new_word );
	  ...
	  if ( !halt_cutting )
	    {
	      ...
	      success = get_affix( SUFFIX, text, &local_cut, affixlen, &c_new_word );
	    }
	}
      
      if ( success )
	{
	  /* Look up the c_new word in the dictionary */
	  *c_new_dictword = lookup_indexed_word( LOOKUP_MAX_TWICE, indexlist, dict, c_new_word, 
	  ...
	  if ( *c_new_dictword == NULL )
	    {
	      ...
	      if ( !skip_added_initial )
	        {
		  *c_new_dictword = lookup_indexed_added_initial( indexlist, dict, c_new_word, 
  ...
  if ( (*c_new_dictword != NULL) && ((mode == SMART_PREFIX) || (mode == SMART_SUFFIX)) )
    {
      if ( (smart_cut = get_smart_cut( (*c_new_dictword)->text, local_cut )) != NULL )
        {
	  free( local_cut );
...
{
  BOOL       success = FALSE;
  char      *c_new_suffix_word, *c_new_prefix_word;

  *suffix_dictword = NULL;
  *prefix_dictword = NULL;

  /* Get the suffix word by removing the prefix */
  success = split_word( PREFIX, (text+testpos), &c_new_prefix_word, splitlen, 
			&c_new_suffix_word, NO_SEPARATOR );

  if ( success )
    {
      /* Look up the c_new word in the dictionary */
      *suffix_dictword = lookup_indexed_word( LOOKUP_MAX_ONCE, features->indexlist, dict, 
					      c_new_suffix_word, NO_TESTPOS, NO_PARTIAL, trans, 
					      dbp, asc );
      ...
      if ( *suffix_dictword != NULL )
	{
	  ...
	  *prefix_dictword = lookup_indexed_word( LOOKUP_MAX_TWICE, features->indexlist, dict, 
						  c_new_prefix_word, testpos, NO_PARTIAL, trans, 
						  dbp, asc );
		     ...
		     DictWord *c_new_dictword, Trans *trans, DB *dbp, Dict *asc )
{
  BOOL  success = FALSE;
  char *prefix, *suffix;
  char *c_new_word, *local_cut = NULL;

  /* Initialize the dictword */
  *c_new_dictword = NULL;

  success = get_root( (text+testpos), &c_new_word, preflen, sufflen, &prefix, &suffix );

  if ( success )
    {
      *c_new_dictword = lookup_indexed_word( LOOKUP_MAX_TWICE, features->indexlist, dict, c_new_word, 
		       ...
		       char **c_new_suffix_word )
{
  ...
  success = split_word( PREFIX, (vanilla_text+testpos), c_new_prefix_word, (division_pos-testpos), 
			c_new_suffix_word, SKIP_SEPARATOR );
		      ...
		      DB *dbp, Dict *asc )
{
  ...
  if ( add_init )
    {
      c_new_dictword = lookup_indexed_added_initial( features->indexlist, dict, c_new_word, testpos, 
						   min_partial_len, trans, dbp, asc );
    }
  else
    {
      c_new_dictword = lookup_indexed_word( mode, features->indexlist, dict, c_new_word, testpos, 
					  min_partial_len, trans, dbp, asc );
    }
  ...
  if ( compress && (c_new_dictword == NULL) )
    {
      ...
      if ( add_init )
	{
	  c_new_dictword = lookup_indexed_added_initial( features->indexlist, dict, c_new_compword, 
						       testpos, min_partial_len, trans, dbp, asc );
	}
      else
	{
	  c_new_dictword = lookup_indexed_word( mode, features->indexlist, dict, c_new_compword, 
					      testpos, min_partial_len, trans, dbp, asc );
	}
    }

  return c_new_dictword;
}
...
void add_transforms( TagTrans *start, TagScore orig_taglist, TagScore c_new_taglist )
{
  ...
  /* Create c_new entry */
  Allocate(tag_list, sizeof(TransFormSt), "transform chain");
  tag_list->source_tag = c_new_taglist;
...
TagCut add_cut( BOOL true_capital, BOOL pseudo_capital, BOOL mixed_capital, char *cut, CutList *cutlist, TagScore vanilla_orig_taglist, TagScore vanilla_c_new_taglist )
{
  TagCut     s;
  char      modifier[MAX_MODIFIER_LEN], *modified_cut;
  ...
  add_transforms( &(s->transform_list), vanilla_orig_taglist, vanilla_c_new_taglist );
...
static int hashword_tag(char *hwrd, TagList *taglist)
{
...
static int hashword_aff(char *hwrd, AffList *afflist)
{
    unsigned int starthash, hash, j;
    char   ch;
...
static int hashword_cut(char *hwrd, CutList *cutlist)
{
    unsigned int starthash, hash, j;
    char   ch;
...
static int hashword_index(char *hwrd, IndexList *indexlist)
{
    unsigned int starthash, hash, j;
    char   ch;
