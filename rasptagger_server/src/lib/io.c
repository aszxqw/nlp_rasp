...
static TextType fetch_text(FILE *fp, Dict *skip_dict, char *text, int maxword,
...
typedef struct
{
	int total;		/* Total words in a c_class */
	int correct;	/* Number which were correctly labelled */
} WordStats;

static struct		/* Counts in each c_class */
{
	WordStats total;
	WordStats known;
	WordStats unknown;
...
void get_stats( BOOL correct, int *stats_correct, int *stats_total, int *stats_correct_in_hyp, Tag ctag, Link unktag_link, char **feature_info, Dict *statdict, BOOL *correct_in_hyp )
{
	int  i;
	Link next_tag;
...
static void log_statistics(BOOL ambig, WordKind kind, BOOL correct, Tag ctag, Link unktag_link, char **feature_info, BOOL *correct_in_hyp)
{
	stats.total.total += 1;
...
static void output_statistics(FILE *fp, char *text, StatsKind c_class)
{
	int total = 0;
	int correct = 0;
	...
	if (c_class == stats_ambig || c_class == stats_unknown)
	{
		char buffer[50];
			...
			Node *node_out)
{
	...
	char **feature_info = node->base->feature_info;
	Link  unktag_link = node->base;
	char *unktext = (node->lex)->word->text;
...
#endif
	{
		...
		const char *format  = OutOpt(compressed) ? "%s" : "%-50s";
