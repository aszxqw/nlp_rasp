...
static BOOL dict_getword(FILE *fp, char *word, int *ntags)
{
...
static int hashword(char *hwrd, Dict *dict)
{
...
DictWord find_word(Dict *dict, char *word, BOOL create)
{
    int      hash;
    DictWord d;
...
----------------------------------------------------------------------------*/
DictWord diction_add_word(Dict *dict, char *text, Tag *tag,
...
void read_dict(FILE *fp, Dict *dict, int size)
{
	...
	char word[MAXWORD];	/* Buffer for a word */
...
void infer_tags(char *infer, Dict *dict)
{
    int i;
    Score total_freq = 0;
    DictWord *k;
...
{DictWord *key;
 ...
 temp->text = (char *)malloc(sizeof(char)*(strlen(d->text) + 1));
...
fprintf(stderr,"In \"%s\" size %d, out \"%s\" size %d, ret value %d\n",dbkey.data,dbkey.size,dbvalue.data,dbvalue.size,ret);
       ...
       if (dbvalue.data != NULL){
           ...
           key = fill_in((char *)dbvalue.data, d, trans);
       }
       else key = NULL;
   }
