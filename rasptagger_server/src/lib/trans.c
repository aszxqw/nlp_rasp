...
/*----------------------------------------------------------------------------
    ...
    If c_new is non-NULL, it is taken as the source of the values; the values
    are NOT cleared.
-----------------------------------------------------------------------------*/

void adjust_trans(Trans *t, Trans *c_new)
{
    int   i, j, size = t->size;
    Score max = -1;
    Score *pi    = t->pi, *frompi = (c_new != NULL) ? c_new->pi : t->pi;
    Score *gamma = t->gamma;
    Score *trans = t->trans, *fromtrans = (c_new) ? c_new->trans : t->trans;
