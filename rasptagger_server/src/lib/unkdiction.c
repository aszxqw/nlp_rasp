...
static int dict_partial_compare(const void *c1, const void *c2)
{
  ...
  char    *dict_text, *search_text, *orig_text, *comptext;
  ...
  Allocate(dict_text, (len2+1)*sizeof(char), "dict_text: dict_partial_compare");
...
DictWord lookup_word( int mode, Dict *dict, char *c_new_word, int testpos, int min_partial_len, 
		      Trans *trans, DB *dbp, Dict *asc )
{
  ...
  if ( c_new_word != NULL )
      ...
      for ( i = 0; (i <= max_search) && (key == NULL); i++ )
	{
	  ...
	  search.text = (c_new_word+i);

	  /* Determine if a partial or full lookup */
	  if ( min_partial_len > 0 )
	    {
	      if ( strlen((char *)(c_new_word+i)) >= (size_t)min_partial_len )
		{
		  exwords.testpos = i;
...
/*----------------------------------------------------------------------------
    ...
    Add a '*' to the c_new_word and call lookup_word.
----------------------------------------------------------------------------*/
			       ...
			       Trans *trans, DB *dbp, Dict *asc )
{
  ...
  if ( !testpos )   /* c_new_word is not sentence-initial */
    {
      test_text = add_chars( c_new_word, "*" );
  ...
  if ( c_new_word != NULL )
    {
      ...
      textlen = (int)strlen((char *)(c_new_word+testpos));
      has_numbers = contains_numbers( c_new_word );
      has_alpha = contains_alpha( c_new_word );

      if ( (textlen <= MinTestLen) || has_numbers || !has_alpha )
        {
	  return lookup_word( mode, dict, c_new_word, testpos, partial_len, trans, dbp, asc );
	}
      ...
      else if ( mode == LOOKUP_MAX_ONCE )
	{
      ...
      is_initial( c_new_word, &index_testpos );
      this_index = get_index( indexlist, (c_new_word+index_testpos) );
      if ( this_index != NULL )
        {
	  for ( i = startpos; (i <= max_search) && (key == NULL); i++ )
	    {
		  ...
		  if ( (compare = strcmp((char *)(scan_text+j), (char *)(c_new_word+i))) >= 0 )
		    {
		      break;
		    }
	        }
				       ...
				       int testpos, int partial_len, Trans *trans, DB *dbp, Dict *asc )
{
  ...
  /* Add a '*' and lookup again */
  if ( (!testpos) && features->initials_exist )   /* c_new_word is not sentence-initial */
    {
      test_text = add_chars( c_new_word, "*" );
...
void add_unkword(Dict *dict, char *text, BOOL skip_closed, TagScore tagscore, char **feature_info)
{
    int i, numfeatures;
    DictWord d;
	...
	for ( numfeatures = 0; (numfeatures < MAXFEATURES) && (feature_info[numfeatures] != NULL); numfeatures++ ) ;
      ...
      if ( numfeatures != 0 )
        {
	  Allocate(d->feature_info, (numfeatures+1) * sizeof(char *), "feature_info: add_unkword");
...
/*----------------------------------------------------------------------------
    ...
    Set up the unknown word tags, by scanning the map list for open c_class
	...
	for ( tag-- ; tag >= 0 ; tag--)
	  {
		...
		Allocate(c_new, sizeof(TagScoreSt), "tag chain");
		c_new->next  = t;
		c_new->tag   = tag;
		c_new->score = 1.0 / total;
		t          = c_new;
	      }
	  }
	...
	for ( i = 0; i < (&(features->unigramdict))->size; i++, u++ )
	  {
		...
		Allocate(c_new, sizeof(TagScoreSt), "tag chain");
		c_new->tag   = m->tag->tag;
		c_new->score = m->tag->score / features->unigram_open_total;
		add_chain_tag( ADD, &(d->tag), c_new, "set_unknown" );
		add_chain_tag( ADD, &(d->unktag), c_new, "set_unknown" );
